...
// const jumpAudio = loadSound('maze-assets/sounds/jump.mp3', false, 0.5);
// const musicAudio = loadSound('maze-assets/sounds/twistMusic.mp3', true, 0.2);
// const swishAudio = loadSound('maze-assets/sounds/swish.mp3', false, 0.4);
// const splatAudio = loadSound('maze-assets/sounds/impactSplat.mp3', false, 0.4);
...
//code 6 trial splat on speed works
// 'use client';
...
// const swishAudio = loadSound('maze-assets/sounds/swish.mp3', false, 0.4);
// const splatAudio = loadSound('maze-assets/sounds/impactSplat.mp3', false, 0.4);
// const mozart = loadSound('maze-assets/sounds/mozart.mp3', true, 0.3);
// const laser = loadSound('maze-assets/sounds/Laser.mp3', false, 0.3);
// const gameOverAudio = loadSound('maze-assets/sounds/gameover.mp3', false, 0.7); // or whatever volume you prefer


// export default function Character({ onModelRef, wallBoxes, keys, lightOn, musicMuted, onGameOver }) {
//   const { scene, animations: idleAnimations } = useGLTF('/maze-assets/models/idlePaul.glb');
//   const modelRef = useRef();
//   const mixerRef = useRef();
//   const [actions, setActions] = useState({});
...
//   const musicStarted = useRef(false);
//   const wasRunning = useRef(false);
//   const loseCoins = useStore((state) => state.loseCoins); // ✅
//   const resetCoins = useStore((state) => state.resetCoins);
...
//   if (!musicStarted.current && (keys.forward || keys.jump || keys.run)) {
//     // if (!musicMuted) musicAudio?.play();
//     if (!musicMuted) mozart?.play();
//     musicStarted.current = true;
//   }

//   mozart.muted = musicMuted;
...
//     const nx = Math.round(position.x) + dx;
//     const nz = Math.round(position.z) + dz;
//     if (mazeGrid[nz]?.[nx] !== 0) {
//       const wallPos = new THREE.Vector3(nx, position.y, nz);
//       if (position.distanceTo(wallPos) < threshold) {
//         nearWall = true;
//         break;
//       }
//     }
...
//   if (coins - 10 <= 0) {
//     resetCoins(); // Reset coins only
//     gameOverAudio?.play(); // ✅ Play game over sound
//     console.log("💀 Game over sound played. Coins reset.");
//     return;
//   }
...
const mozart = loadSound('maze-assets/sounds/mozart.mp3', true, 0.3);
const laser = loadSound('maze-assets/sounds/Laser.mp3', false, 0.3);
const gameOverAudio = loadSound('maze-assets/sounds/gameover.mp3', false, 0.7);
...
export default function Character({ onModelRef, wallBoxes, keys, lightOn, musicMuted, onGameOver }) {
  ...
  useEffect(() => {
    if (!Object.keys(actions).length || isDead) return;
  ...
  useFrame((_, delta) => {
    ...
    if (keys.run && !wasRunning.current) swishAudio?.play();
    wasRunning.current = keys.run;
    ...
    if (nearWall) {
      ...
      if (coins - 10 <= 0 && !isDead) {
        mozart?.pause(); // ✅ Stop Mozart
        gameOverAudio?.play(); // ✅ Play game over sound
        setIsDead(true); // ✅ Block further updates
        onGameOver?.(); // ✅ Trigger overlay
