...
//   const calculateSampleSize = (p = 0.5, E = 0.05, N) => {
//     const Z = 1.96; // For 95% confidence level
//     const n = Math.ceil((Z * Z * p * (1 - p)) / (E * E)); // Sample size formula
//     if (N && N > 0) {
//       // Apply finite population correction if N is non-zero
//       return Math.ceil((n * N) / (N + n - 1));
//     }
//     return n; // Return n as it is if N is not provided or is zero
//   };
...
//     const totalKnown = Object.values(knownWords).reduce((a, b) => a + b, 0);
//     const totalTested = testedWords.length;
//     const totalWords = words.length;//total number of words available
...
//   const calculateWordsForSelectedLevels = (levels, words) => {
//     // Filter the words that match the selected CEFR levels
//     const selectedWords = words.filter((word) => levels.includes(word.CEFR));
//     return selectedWords.length; // Return the number of words for the selected levels
//   };
