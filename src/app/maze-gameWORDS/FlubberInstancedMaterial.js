  ...
  constructor() {
    super({
      uniforms: {
        uTime: { value: 0 },
        uResolution: { value: new THREE.Vector2(1, 1) },
        uCenter: { value: new THREE.Vector2(15.0, 15.0) }
      },
      vertexShader: `
        varying vec2 vUv;
        varying vec3 vWorldPosition;
...
void main() {
  // Scale UVs to control brick size
  vec2 brickUV = vUv * vec2(12.0, 6.0);

  // Shift every other row
  float rowShift = mod(floor(brickUV.y), 2.0) * 0.5;
  brickUV.x += rowShift;

  // Find cell and its border
  vec2 cell = fract(brickUV);
  float mortarX = step(0.94, cell.x) + step(cell.x, 0.06);
  float mortarY = step(0.94, cell.y) + step(cell.y, 0.06);
  float mortar = clamp(mortarX + mortarY, 0.0, 1.0);

  // Brick and mortar colors
  vec3 brickColor = vec3(0.85, 0.4, 0.1);     // orange brick
  vec3 mortarColor = vec3(0.15, 0.15, 0.15);  // dark grey mortar

  vec3 finalColor = mix(brickColor, mortarColor, mortar);
